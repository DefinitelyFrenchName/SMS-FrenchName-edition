...
| 10. In-match combo counter     | Live combo-hit counter rendered by the base game under each attacker's bar (no overlay needed; 2026-07-25 fix: now also shows vs the CPU) | `tools/mkpatch10.py`                 | `build/sms_combocounter.bps`                           | `be072a5e…`   |
| 10b. + status labels      | Counter + GC/REVERSAL/PUNISH/TECH event text (MEATY label removed 2026-07-20; 2026-07-25 fix: labels now expire correctly) | `tools/mkpatch10.py --events labels` | `build/sms_combolabels.bps`                            | `920652df…`   |
| 11. Training+  **(not fully tested)**                 | In-ROM training-mode upgrade: L+R menu, dummy control (pose/guard/wakeup/tech), recording+playback, damage/regen/refill, input+ADV display | `tools/mkpatch11.py`                 | `build/sms_trainingplus.bps`                           | `e9ac2205…`   |
