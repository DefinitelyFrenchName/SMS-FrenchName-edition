...
local log = assert(io.open(TRACE.."probe_dshit.txt","w"), "probe_dshit.lua: cannot open " .. (TRACE.."probe_dshit.txt"))
emu.addMemoryCallback(function()
  if needLoad then local f = io.open(TRACE.."neptune_vs_jupiter.mss","rb") if not f then print("probe_dshit.lua: cannot open " .. (TRACE.."neptune_vs_jupiter.mss")) emu.stop(1) return end
