...
ctxRef.onFirstExec=function(ctx) local f = io.open(TRACE.."neptune_vs_jupiter.mss","rb") if not f then print("probe_viewer.lua: cannot open " .. (TRACE.."neptune_vs_jupiter.mss")) emu.stop(1) return end ctx.anchor.loadreq=f:read("*a"); f:close() end
table.insert(ctxRef.hooks.frame, function(ctx)
  ...
  if t==(SHOT_AT or 100) then local f = io.open(TRACE..(SHOT_OUT or "viewer.png"),"wb") if not f then print("probe_viewer.lua: cannot open " .. (TRACE..(SHOT_OUT or "viewer.png"))) emu.stop(1) return end f:write(emu.takeScreenshot()); f:close(); emu.stop(0) end
end)
print("viewer loaded")
