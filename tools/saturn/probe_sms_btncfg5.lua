...
local rlog = 0
for _, a in ipairs({0x7E1846, 0x001846}) do
  emu.addMemoryCallback(function(addr, value)
    if cfg0 == nil or frames < cfg0 + 140 or rlog > 30 then return end
    rlog = rlog + 1
    local ok, st = pcall(emu.getState)
    local pc = st and (st["cpu.pc"] or st["snes.cpu.pc"]) or -1
    local k = st and (st["cpu.k"] or st["snes.cpu.k"]) or -1
    log(string.format("f=%d R %06X = %02X @ %02X:%04X", frames, addr, value or -1, k, pc))
  end, emu.callbackType.read, a, a, emu.cpuType.snes, emu.memType.snesMemory)
end
emu.addMemoryCallback(function(addr, value)
  if cfg0 == nil then return end
  local ok, st = pcall(emu.getState)
