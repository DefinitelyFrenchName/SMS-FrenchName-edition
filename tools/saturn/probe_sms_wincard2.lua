...
emu.addEventCallback(function()
  ...
  t = t + 1
  local CHAR = tonumber(os.getenv("CHAR") or "0", 16)
  if t == 60 then
    if CHAR > 0 then wr(0x1000, CHAR) else wr(0x1000, 0x1C) end
  ...
  if t == 2000 then
    ...
    local f = assert(io.open(ENV.TRACE .. "saturn/vram_card_" .. (os.getenv("TAG") or "van") .. ".bin", "wb"))
    local b = {}
