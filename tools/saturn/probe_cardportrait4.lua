...
-- probe_cardportrait4.lua — cardportrait flow + the $9F:84E8 decompressor hook;
-- while the card builds it marks every 512-byte ROM block read across $80-$FF and
-- logs the sorted "ROMBLOCKS" list at dump time, to corner the portrait's source data.
...
emu.addMemoryCallback(function(addr)
  if watching then blocks[addr >> 9] = true end
end, emu.callbackType.read, 0x800000, 0xFFFFFF, emu.cpuType.snes, emu.memType.snesMemory)
local dseen = 0
emu.addMemoryCallback(function(addr)
...
local function dumpvram(tag)
  ...
  local ks = {}
  for k in pairs(blocks) do ks[#ks+1] = k end
  table.sort(ks)
  local out = {}
  for _, k in ipairs(ks) do out[#out+1] = string.format("%06X", k * 512) end
  log("ROMBLOCKS " .. table.concat(out, " "))
  log("dumped " .. tag)
end
local STEPS = {
