...
emu.addMemoryCallback(function(addr, value)
  local off = addr % 0x200
  if not hits[off] then
    hits[off] = frames
    log(string.format("f=%d first write 7F:%04X <= %02X", frames, 0xF000 + off, value or -1))
  end
end, emu.callbackType.write, 0x7FF000, 0x7FF1FF, emu.cpuType.snes, emu.memType.snesMemory)
local STEPS = {
...
emu.addEventCallback(function()
  ...
  if not STEPS[step] then
    ...
    log(string.format("TOTAL touched in $7F:F000-F1FF: %d/512", n))
    emu.stop(0)
  end
