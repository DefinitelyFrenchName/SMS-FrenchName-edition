-- probe_sms_charsel.lua — map the char-select engine on vanilla SMS:
...
-- ROM=<clean SMS> tools/run.sh tools/saturn/probe_sms_charsel.lua 300
local ENV = dofile((package.path:match("([^;]+)%?%.lua$") or error("sms_env: tools dir not in package.path")) .. "/../sms_env.lua")
local PL = ENV.dofile("probelib.lua")
local LOG = assert(io.open(ENV.TRACE .. "saturn/charsel.txt", "w"))
...
local STEPS = {
  ...
  function() return sf>240 end,                       -- now at VS char select
  ...
  function()  -- poke P1 cursor to 10, observe
    wr(0x1B40, 10)
    return sf > 2
  end,
  function()
    if sf % 30 == 0 then
      log(string.format("f=%d POKE10 p1cur=%02X p2cur=%02X pc=%02X:%04X sfx78=%02X",
        frames, ram(0x1B40), ram(0x1B80), st("k"), st("pc"), ram(0x78)))
    end
    return sf > 120
  end,
  function() oamdump("-cur10"); return true end,
  function()  -- restore a sane cursor, then confirm with A to capture confirm-exec context
    wr(0x1B40, 6)
    pulse[0] = beat({a = true})
    if ram(0x1B42) == 1 then log(string.format("f=%d CONFIRMED p1cur=%02X", frames, ram(0x1B40))); return true end
    return sf > 120
  end,
  function() pulse[0] = {}
             log(string.format("FINAL p1cur=%02X p2cur=%02X conf=%02X pc=%02X:%04X",
               ram(0x1B40), ram(0x1B80), ram(0x1B42), st("k"), st("pc")))
             return true end,
}
