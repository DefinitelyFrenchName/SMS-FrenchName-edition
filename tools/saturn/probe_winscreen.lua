...
-- win-screen instrumentation: our list hook must NOT act there
local ws, wslog = 0, {}
emu.addMemoryCallback(function()
  if ws > 40 then return end
  local ok, st = pcall(emu.getState)
  local x = st and (st["cpu.x"] or 0) or 0
  local bk = emu.read(0x7E0000 + x + 0x66, emu.memType.snesMemory)
  local hi = emu.read(0x7E0000 + x + 0x65, emu.memType.snesMemory)
  local lo = emu.read(0x7E0000 + x + 0x64, emu.memType.snesMemory)
  local key = string.format("f=%d X=%04X ptr=%02X:%02X%02X card=%s", frames, x, bk, hi, lo,
    tostring(cardnow))
  ws = ws + 1
  wslog[#wslog + 1] = key
end, emu.callbackType.exec, 0x809E86, 0x809E86, emu.cpuType.snes, emu.memType.snesMemory)

local function dumpvram(tag)
  for _, l in ipairs(wslog) do log("LISTHOOK " .. l) end
...
local STEPS = {
  ...
  function()   -- watch the WIN SCREEN before the card
    if sf % 45 == 0 and sf <= 450 then
      local png = emu.takeScreenshot()
      local f = assert(io.open(ENV.TRACE .. "saturn/win_" .. TAG .. "_" .. sf .. ".png", "wb"))
      f:write(png); f:close()
      log(string.format("WINSHOT sf=%d INIDISP=%02X 1E05=%02X 70=%02X", sf,
        emu.read(0x802100, emu.memType.snesMemory), ram(0x1E05), ram(0x70)))
    end
    return sf > 460
  end,
