...
emu.addEventCallback(function()
  ...
  if not STEPS[step] then
    ...
    local fo = assert(io.open(ENV.TRACE .. "saturn/vram_fx.bin", "wb"))
    local bb = {}
    for i = 0, 0x103F do bb[#bb+1] = string.char(emu.read(0x6A00 * 2 + i, V)) end
    fo:write(table.concat(bb)); fo:close()
    log("vram dumped")
    log("done"); emu.stop(0)
  end
