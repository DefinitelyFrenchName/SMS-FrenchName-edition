...
-- HEAL: blank the winner variable exactly at card-build time, so the build-time
-- gate fails and only the per-frame path can rescue the portrait
if os.getenv("HEAL") == "1" then
  for _, a in ipairs({0xEEC900, 0xF6C900}) do
    emu.addMemoryCallback(function()
      wr(0x1E14, 0)
      log("HEAL: zeroed $1E14 at the card-build hook")
    end, emu.callbackType.exec, a, a, emu.cpuType.snes, emu.memType.snesMemory)
  end
end
...
emu.addEventCallback(function()
  ...
  -- HEAL=1 reproduces "the winner is not latched yet at card-build time":
  -- $7E:1E14 is held at 0 until the card is actually on screen, then set.
  if os.getenv("HEAL") == "1" and cardnow then wr(0x1E14, 1) end
  local fn = STEPS[step]
