-- probe_lrboth_dbg.lua — L+R held on BOTH pads through Uranus-vs-Jupiter select:
-- debug the $E8:2980 DMA stub (entry regs, in-stub step probes, $80:92A4 kicks,
-- $7F:F100-F103 flag writes) plus effect-tile staging/DMA watchers; verdict
-- LR-BOTH PASS/FAIL (both players become Saturn, id 0x1C).
-- ROM=<Saturn build> tools/run.sh tools/saturn/probe_lrboth_dbg.lua 300
local ENV = dofile((package.path:match("([^;]+)%?%.lua$") or error("sms_env: tools dir not in package.path")) .. "/../sms_env.lua")
local PL = ENV.dofile("probelib.lua")
local LOG = assert(io.open(ENV.TRACE .. "saturn/lrboth_dbg.txt", "w"))
local function log(s) LOG:write(s .. "\n"); LOG:flush() end
local ram, wr = PL.ram, PL.wr
local nstub, nkick = 0, 0
emu.addMemoryCallback(function()
  nstub = nstub + 1
  if nstub <= 40 then
    local okk, stt = pcall(emu.getState)
    local D = stt and (stt["cpu.d"] or stt["snes.cpu.d"]) or -1
    local DB = stt and (stt["cpu.db"] or stt["snes.cpu.db"]) or -1
    local PS = stt and (stt["cpu.ps"] or stt["snes.cpu.ps"]) or -1
    log(string.format("  D=%04X DB=%02X P=%02X", D, DB, PS))
    log(string.format("DMASTUB entry: $30=%02X%02X $36=%02X pad4218=%02X flag=%02X latch=%02X",
      ram(0x31), ram(0x30), ram(0x36),
      emu.read(0x804218, emu.memType.snesMemory),
      emu.read(0x7FF100, emu.memType.snesMemory),
      emu.read(0x7FF102, emu.memType.snesMemory)))
  end
end, emu.callbackType.exec, 0xE82980, 0xE82980, emu.cpuType.snes, emu.memType.snesMemory)
emu.addMemoryCallback(function()
  nkick = nkick + 1
end, emu.callbackType.exec, 0x8092A4, 0x8092A4, emu.cpuType.snes, emu.memType.snesMemory)
for _, off in ipairs({0x15, 0x20, 0x2A, 0x2C, 0x40, 0x50}) do
  emu.addMemoryCallback(function()
    local okk, stt = pcall(emu.getState)
    local A = stt and (stt["cpu.a"] or stt["snes.cpu.a"]) or -1
    log(string.format("  step +%02X A=%04X", off, A))
  end, emu.callbackType.exec, 0xE82980 + off, 0xE82980 + off, emu.cpuType.snes, emu.memType.snesMemory)
end
emu.addMemoryCallback(function(addr, value)
  local ok, st = pcall(emu.getState)
  local pc = st and (st["cpu.pc"] or st["snes.cpu.pc"]) or -1
  local k = st and (st["cpu.k"] or st["snes.cpu.k"]) or -1
  log(string.format("WFLAG %06X <= %02X @ %02X:%04X", addr, value or -1, k, pc))
...
emu.addEventCallback(function()
  ...
  if not STEPS[step] then
    ...
    log(string.format("stub entries=%d kicks=%d", nstub, nkick)); log("done"); emu.stop(0)
  end
