...
local pseen = 0
emu.addMemoryCallback(function()
  if pseen > 6 then return end
  pseen = pseen + 1
  local ok, st = pcall(emu.getState)
  local A = st and (st["cpu.a"] or st["snes.cpu.a"]) or -1
  log(string.format("PORTRAIT-LOOKUP A=%04X (p1id=%02X p2id=%02X 1B40=%02X 1B80=%02X)",
    A, ram(0x1000), ram(0x1080), ram(0x1B40), ram(0x1B80)))
end, emu.callbackType.exec, 0x9F9487, 0x9F9487, emu.cpuType.snes, emu.memType.snesMemory)
local entries, nent = {}, 0
emu.addMemoryCallback(function(addr)
  if not watching or nent > 12 then return end
  local a = addr & 0xFFFF
  if entries[a] then return end
  entries[a] = true; nent = nent + 1
  local ok, st = pcall(emu.getState)
  local sp = st and (st["cpu.sp"] or st["snes.cpu.sp"]) or 0
  local r = emu.read(sp + 1, emu.memType.snesMemory) + 256 * emu.read(sp + 2, emu.memType.snesMemory)
  local rb = emu.read(sp + 3, emu.memType.snesMemory)
  log(string.format("BANK9F first-exec $%04X (stack ret %02X:%04X)", a, rb, r))
end, emu.callbackType.exec, 0x9F0000, 0x9FFFFF, emu.cpuType.snes, emu.memType.snesMemory)
local dseen = 0
emu.addMemoryCallback(function(addr)
