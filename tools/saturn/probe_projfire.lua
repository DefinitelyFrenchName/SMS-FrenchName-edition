-- probe_projfire.lua — find the code that draws the in-match NAMEPLATE (the
...
--
--   SHELL_ID=7 SATURN=1 ROM=<build> tools/run.sh tools/saturn/probe_projfire.lua 700
...
local shot, live = false, 0
local acts, s1, s2 = {}, 0, 0
...
local STEPS = {
  ...
  function()
    -- PRACTICE (row 4) is the only mode that works across the whole 0.14.1-0.14.6
    -- range: her inputs are dead in vs-COM and 2P VS until the v0.14.3 fix, which
    -- is why a vs-COM run showed act $22 for 702 frames and never fired.
    if num("ROW", 4) ~= 4 then return true end
  ...
  function()
    -- A REAL round in vs-COM (the nameplate work proved this autopilot reaches
    -- one). Practice-mode captures earlier in this investigation had no HUD and,
    -- as it turns out, were taken during the round intro when she cannot act —
    -- which is why "the projectile slot is populated" never coincided with a
    -- projectile on screen. Wait for the round proper, then throw 214+LP on a
    -- loop and shoot the frame the projectile is actually alive.
    if sf < 200 then pulse[0] = {}; return false end
    local m = sf % 50
    ...
    -- instrument check: is she ACTING at all, and does either projectile slot
    -- ever populate? "no capture" must not be confused with "she never fired".
    acts[ram(0x1001)] = (acts[ram(0x1001)] or 0) + 1
    if ram(0x1100) ~= 0 then s1 = (s1 or 0) + 1 end
    if ram(0x1180) ~= 0 then s2 = (s2 or 0) + 1 end
    local id = ram(0x1100)
    if id ~= 0 and not shot then
      live = (live or 0) + 1
      if live >= 6 then                      -- a few frames in, so it has travelled
        shot = true
...
emu.addEventCallback(function()
  ...
  if not STEPS[step] then
    ...
    log(string.format("TOTAL VRAM port writes: %d ; in nameplate window $%04X-$%04X: %d",
      total, LO, HI, inwin))
    if not shot then log("NO PROJECTILE CAPTURED — harness, not a finding") end
    log(string.format("slot $1100 non-zero on %d frames; $1180 on %d", s1 or 0, s2 or 0))
    local aa = {}
    for k, v in pairs(acts) do if v > 8 then aa[#aa + 1] = string.format("%02X:%d", k, v) end end
    table.sort(aa)
    log("P1 acts seen (>8f): " .. table.concat(aa, " "))
