...
local STEPS = {
  ...
  function()   -- screenshot each kick variant mid-active
    local CASES = {{28, "a", "close5HK"}, {44, "a", "far5HK"}, {24, "b", "5LK"}}
    local slot = math.floor(sf / 80) + 1
    local c = CASES[slot]
    if c == nil then return true end
    local ph = sf % 80
    if ph == 1 then
      wr(0x1021, 0x60); wr(0x1022, 0); wr(0x10A1, 0x60 + c[1]); wr(0x10A2, 0)
    elseif ph >= 5 and ph <= 8 then
      pulse[0] = {[c[2]] = true}
    else
      pulse[0] = {}
    end
    if ph == 16 or ph == 22 or ph == 28 then
      local png = emu.takeScreenshot()
      local f = assert(io.open(ENV.TRACE .. "saturn/mv_" .. c[3] .. "_" .. ph .. ".png", "wb"))
      f:write(png); f:close()
      log(string.format("SHOT %s ph=%d act=%02X pose=%02X", c[3], ph, ram(0x1001), ram(0x1005)))
    end
    return false
  end,
