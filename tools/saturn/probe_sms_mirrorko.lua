...
local kophase = nil
local wlog = 0
for _, a in ipairs({0x7E10A5, 0x0010A5, 0x7E10B2, 0x0010B2}) do
  emu.addMemoryCallback(function(addr, value)
    if kophase and (frames - kophase) > 160 and wlog < 20 then
      wlog = wlog + 1
      local ok, st = pcall(emu.getState)
      log(string.format("  W %06X <= %02X @ %02X:%04X", addr, value or -1,
...
local ecount = {}
for _, spec in ipairs({{0xC10083, "integrator-call"}, {0xEFC6F7, "sat-dispatch"},
                       {0xEFD2D2, "act1E-handler"}, {0xEFDB70, "helper"}}) do
  emu.addMemoryCallback(function()
    if kophase and (frames - kophase) > 160 then
      ecount[spec[2]] = (ecount[spec[2]] or 0) + 1
    end
...
emu.addEventCallback(function()
  ...
  if not STEPS[step] then
    ...
    if not kophase then
      ...
      kophase = frames
      log("-- entering KO phase")
    end
  end
  if kophase then
    local e = frames - kophase
    if e == 30 then
      wr(0x10C9, 1)                                   -- P2 to 1 HP
      wr(0x1021, 0x90); wr(0x1022, 0x00)              -- adjacent
      wr(0x10A1, 0xA4); wr(0x10A2, 0x00)
    end
    if e >= 40 and e < 200 then pulse[0] = (frames % 12 < 3) and PL.pad({y=true}) or PL.pad() end
    if e >= 40 and e % 15 == 0 then
      log(string.format("ko+%d: p1 act=%02X | p2 act=%02X st=%02X pose=%02X y=%02X%02X vy=%02X%02X f16=%02X hp=%d | 1E05=%02X",
        e, ram(0x1001), ram(0x1081), ram(0x1082), ram(0x1085),
        ram(0x10A6), ram(0x10A5), ram(0x10B3), ram(0x10B2), ram(0x1096),
        ram(0x10C9), ram(0x1E05)))
    end
    if e == 600 then
      local png = emu.takeScreenshot()
      local f = assert(io.open(ENV.TRACE .. "saturn/mirrorko.png", "wb"))
      f:write(png); f:close()
      local parts = {}
      for k, v in pairs(ecount) do parts[#parts+1] = k .. "=" .. v end
      table.sort(parts)
      log("exec counts (last ~440 frames): " .. table.concat(parts, " "))
      log("shot at ko+600"); emu.stop(0)
    end
  end
