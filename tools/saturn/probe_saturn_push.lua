...
local STEPS = {
  ...
  function()   -- park them adjacent, then walk P1 into P2 and watch the gap
    -- drop them on the SAME x: with a valid push box the engine separates them
    if sf == 1 then wr(0x1021, 0x90); wr(0x1022, 0); wr(0x10A1, 0x90); wr(0x10A2, 0) end
    if sf % 10 == 0 then
      local p1 = ram(0x1021) + 256 * ram(0x1022)
      local p2 = ram(0x10A1) + 256 * ram(0x10A2)
      log(string.format("PUSH sf=%d p1x=%04X p2x=%04X gap=%d p1coll=%02X p2coll=%02X",
        sf, p1, p2, p2 - p1, ram(0x1042), ram(0x10C2)))
    end
    return sf > 180
  end,
