...
-- census of every sprite-list pointer the renderer loads, IN MATCH and at the
-- card: is "bank $9F" a safe stand-in for "this is the report card"?
local lp, lpseen, lpn = {}, {}, 0
emu.addMemoryCallback(function()
  if lpn > 4000 then return end
  lpn = lpn + 1
  local ok, st = pcall(emu.getState)
  ...
  local key = string.format("%02X:%02X%02X", bk, hi, lo)
  if not lpseen[key] then
    lpseen[key] = (cardnow and "card" or "match")
    lp[#lp + 1] = key .. "(" .. lpseen[key] .. ")"
  end
