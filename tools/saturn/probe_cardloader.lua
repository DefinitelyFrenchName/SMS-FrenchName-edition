...
-- every call of the portrait loader ($80:8DEC), with its caller
local pl, pllog = 0, {}
for _, a in ipairs({0x808DEC, 0xC08DEC}) do
  emu.addMemoryCallback(function()
    if pl > 12 then return end
    pl = pl + 1
    local ok, st = pcall(emu.getState)
    local sp = st and (st["cpu.sp"] or 0) or 0
    pllog[#pllog + 1] = string.format("f=? dst=%02X%02X ret=%02X:%02X%02X",
      ram(0x04), ram(0x03),
      emu.read(sp + 3, emu.memType.snesMemory),
      emu.read(sp + 2, emu.memType.snesMemory),
      emu.read(sp + 1, emu.memType.snesMemory))
  end, emu.callbackType.exec, a, a, emu.cpuType.snes, emu.memType.snesMemory)
end
...
local function dumpvram(tag)
  ...
  log("PORTRAIT LOADER: " .. table.concat(pllog, " | "))
