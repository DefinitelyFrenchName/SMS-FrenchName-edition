...
-- probe_saturn_variant.lua — map Saturn's close/far normals: press HK or HP
-- (BTN=a/x) at a sweep of P2 distances and log which act ID comes out at each.
...
local STEPS = {
  ...
  function()   -- act produced by HK / HP at a range of distances
    local DISTS = {16, 24, 28, 32, 36, 44, 56}
    local BTN = os.getenv("BTN") or "a"          -- a = HK, x = HP
    local slot = math.floor(sf / 60) + 1
    local d = DISTS[slot]
    if d == nil then return true end
    local ph = sf % 60
    if ph == 1 then
      wr(0x1021, 0x60); wr(0x1022, 0)
      wr(0x10A1, 0x60 + d); wr(0x10A2, 0)
      seen = nil
    elseif ph >= 5 and ph <= 8 then
      pulse[0] = {[BTN] = true}
    else
      pulse[0] = {}
    end
    local a = ram(0x1001)
    if ph > 8 and a >= 0x40 and seen == nil then
      seen = a
      log(string.format("VARIANT dist=%d button=%s -> act %02X", d, BTN, a))
    end
    return false
  end,
  function() local png = emu.takeScreenshot()
             local f = assert(io.open(ENV.TRACE .. "saturn/push_" .. TAG .. ".png", "wb"))
             f:write(png); f:close(); return true end,
}
emu.addEventCallback(function()
