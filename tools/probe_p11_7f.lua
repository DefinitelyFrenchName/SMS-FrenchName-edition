...
local pulse = {}
local pagesR, pagesW = {}, {}
emu.addMemoryCallback(function(addr) pagesR[math.floor((addr - 0x10000) / 256)] = true end,
  emu.callbackType.read, 0x10000, 0x1FFFF, emu.cpuType.snes, emu.memType.snesWorkRam)
emu.addMemoryCallback(function(addr) pagesW[math.floor((addr - 0x10000) / 256)] = true end,
...
local function report(tag)
  local function fmt(t)
    ...
    table.sort(l)
    local s, n = "", #l
    for i = 1, math.min(n, 40) do s = s .. string.format(" %02X", l[i]) end
    if n > 40 then s = s .. " ..." end
    return n, s
  end
