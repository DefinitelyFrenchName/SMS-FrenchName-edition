...
emu.addMemoryCallback(function()
  if needLoad then local f = io.open(TRACE.."uranus_vs_jupiter_v07.mss","rb") if not f then print("probe_bar.lua: cannot open " .. (TRACE.."uranus_vs_jupiter_v07.mss")) emu.stop(1) return end
...
emu.addEventCallback(function()
  ...
  if not log then print("probe_bar.lua: cannot open " .. (TRACE.."probe_bar.txt")) emu.stop(1) return end
  ...
  if t==110 then log:write(p2bar("HEALED-valueonly disp="..string.format("%02X",emu.read(0x0801,WRAM))).."\n")
    local f = io.open(TRACE.."bar_healed.png","wb")
    if not f then print("probe_bar.lua: cannot open " .. (TRACE.."bar_healed.png")) emu.stop(1) return end
