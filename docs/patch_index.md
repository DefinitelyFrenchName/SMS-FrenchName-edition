...
| 16 | Menu translation | **IN PROGRESS — step 1 (font install) WORKS.** Translate the menu text; a standalone patch, NOT a Saturn feature, and must work with or without her. A complete half-width A-Z (`tools/mkhalfwidth.py`, condensed from the game's own capitals) is installed into the menu font sheet `$C4:2590` (418 -> 512 tiles, relocated) and reaches **VRAM tiles $5C0-$5FF**, verified by reading VRAM back (52/64 non-blank = 26 letters x 2 tiles, vs 0 on clean). The blocker was the asset-record layout: it is `[vram16][len16][src24][dest24]`, so the upload LENGTH is 2 bytes BEFORE the src pointer (field `$C3:BF18`, $3480 -> $4000 bytes) — earlier attempts bumped the wrong record. Step 2 (tilemaps): the **Options** screen WORKS (2026-08-06) — the old blocker was the Options transition clearing ALL of VRAM (fixed-source DMA at `$80:8191`) and its loader (`$C3:A4DD`) not re-uploading the font; the builder now hooks that loader's first load with a 60-byte stub that runs the font record first (always on). `SMS_P16_OPTIONS=1` translates the six labels (COM LEVEL/TIMER/BGM/SFX/VOICES/EXIT, render-verified); budgets measured (18 columns a label, 6 a value; tilemap = asset record 19 `$C3:69F0`); the option VALUES are translated too — the runtime writer is `$80:8C43` drawing self-describing records from bank `$C4` (uncompressed; 12 records edited in place, both highlight palettes). Win/ACS screens not yet probed. Detail: `docs/menu_text.md` | IN PROGRESS | — |
