...
`tools/saturn/verify_saturn.sh` (47 checks) rather than `mkpatchN.py` and the
...
|---|---|---|---|---|
| **100** | SMS + Saturn | Sailor Saturn playable in SMS, summoned by holding **L+R** while confirming a Uranus/Neptune/Pluto slot (she wears that character as a "shell"). A COMPOSITE, like the REF bundles: her four animation layers + per-char proc block, card portrait, push-collision fix, sfx remap, projectile palette split, her own voice (in-match + character-select line), her own movelist, and a Super S stage ported onto Pluto's slot. Build flags: `SATURN_HIDDEN=1` (the only shipped variant), `SATURN_VOICE=0`, `SATURN_PITCH` (= patch 101) | **CURRENT** — v0.14.12 on REF v.2, hidden `a56a9a2e…` / +stage `57332d87…`, feature-complete, no open bugs (v0.14.11 fixed the 214P projectile — the effects DMA was sized from the SHELL, so a Neptune shell truncated her effect sheet by 15 tiles; v0.14.12 makes her four palettes follow the confirm button) | built by `tools/saturn/build_refsaturn.sh` |
