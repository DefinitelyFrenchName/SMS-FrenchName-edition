...
|---|---|---|---|
| Jupiter/Uranus SPD input | command grab | **minimum motion = `6 2 4 8` + P** (Jupiter) or **+ K** (Uranus); the 8 and the button may share a frame, and the attacker must be at CONTACT range or it whiffs. Maintainer-supplied, 2026-08-03; the longer `6321478` used by test_regression.lua also comes out but its up-steps make the attacker jump, so it never lands in a walk-in harness. | maintainer + probe_sms_throwoam.lua |
| $C1:0881 | throw_victim_pose_table | **Per-VICTIM thrown-pose lists — a TEN-entry, 1-indexed pointer table** (idx 0 dead, idx 1-9 = the nine characters). During a throw the THROWER's script drives the VICTIM's pose through it: `jsr $C1:03DC` returns the OTHER object's base, so `$0E` = the victim's charID; `$0E*2` indexes this table; the script record's byte 0 then indexes 21 bytes at that pointer to give the pose, written to the victim's **+0x05** by `sta $0005,y`. Two read sites, byte-identical in shape: **`$C1:0735`** (normal throws) and **`$C1:0C51`** (command/carry throws). Lists live at $0895, $08AA … $093D, 0x15 apart, with code immediately after $0951 — the table cannot be extended in place. Super S's twin is at `$C1:0883` with ELEVEN entries (idx 10 = Saturn), and the nine shared lists are byte-identical across the games. | probe_sms_throwoam.lua + disasm (2026-08-03) |
| $7E:xx05 | object pose | pose index into the char's pose->spritelist table; read by the OAM renderer `$C0:9A0E` (`lda $05,x`, `pose*2`), whose list begins with a **sprite COUNT byte** — so an out-of-range pose makes the emitter write garbage counts and flood OAM (127 visible sprites vs ~50 healthy). | renderer disasm |
...
| $7E:0802 | timer_tick | the 60-frame TICK, not the timer value — reloaded #$3C each wrap (training Lua freezes it). Digits are $0803 units / $0804 tens | measured 2026-08-10 |
...
| $C1:0459 | **normals_starter** | fresh attack button (high nibble of `+0x50`, else `+0x52`) selects one of FOUR 3-byte records at [Y]: `[distance threshold, far act, close act]`; threshold ≥ distance from `$C1:0439` picks the close act, 255 = no proximity variant. Record order `0x10`LP `0x20`LK `0x40`HP `0x80`HK. **Y is the stance's table, which is what makes a command normal** — Uranus stand `$C1:7AF5`, crouch `$C1:7B01`, neutral jump `$C1:7B0D`, directional jump `$C1:7B19` |
| $C1:11E2 | **double_jump** (ChibiMoon only) | 3 call sites, all her jump acts. Needs UP (`+0x50` bit3) and an APEX window `abs(+0x32) < 0x0200`, so it cannot fire while rising or falling fast; sets base/base+1/base+2 by direction vs facing `+0x09` |
| $C1:0BDD | **triangle_jump** (Mercury only) | 2 call sites, her jump fwd/back acts only — not neutral jump. Needs `+0x16` bit6 (wall contact), direction held into the wall and the matching sign of `+0x31`; flips facing `+0x09` then sets the act |
| $C1:0952 | **special_starter** (was `cmd_cancel_commit`) | reads the pending cmd nibble $51/$53,X, indexes (nibble−2)*2 into the table in Y, tests its flag byte, commits the act via $0224. **Two entry points, and the difference is the whole mechanism**: `$C1:0952` requires the connected flag $43,X≠0 — the CANCEL path, i.e. cancelling an attack that hit — and **`$C1:0958` skips that check**, which is what a guard handler calls. Renamed 2026-08-10: "cancel" is right for the `$0952` entry and wrong for the routine, which is reached from 22 of Uranus's 43 universal acts including idle/walk/crouch/prejump/land |
...
| $7E:0802 / $0803 / $0804 | timer_tick / units / **tens** | the clock is three bytes: $0802 ticks 60 frames (decremented at $C0:D68D, reloaded #$3C at $C0:D692), $0803 is the units digit, $0804 the tens; both index HUD digit tiles at +$2C50. $C0:D6A6 raises $1F5C on expiry, after which the routine skips itself. ⚠ **$0804 == 0 = under ten seconds left** — the clock condition the desperation gate reads at $C1:09BF; $1F5C is the EXPIRED flag, not the clock |
