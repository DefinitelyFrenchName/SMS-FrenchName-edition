...
Two of these documents are also published as drawn pages, generated by committed
tools and rendered in CI — never hand-edited, so they cannot drift from the source:
...
A third page, **[how an act is chosen](https://claude.ai/code/artifact/fc6c32ec-753b-459d-bbfe-f69601d766fc)**, draws the start routes: what a
handler can begin (movement, normals, per-character movement, throws, specials)
and the acts nobody chooses (reactions, projectiles, scripted). Text versions are
`sms_engine_internals.md` §2.x and §7.x and `sms_specials.md`.
⚠ Unlike the two above it is a **hand-authored private artifact**, not generated
from this repo and not reachable without access — treat the documents as the
source of truth and the page as an illustration. Promoting it to a generator
beside `tools/mkenginepage.py` is the open option if it is ever wanted publicly.
