...
**The special-start table is one of FOUR start routes an act handler offers**, alongside
the stance's normals table (`$C1:0459`), the close-throw table (`$C1:055A`) and, for two
characters, a movement routine. All four routes — and therefore what makes a normal a
normal and a command normal a command normal — are in
`sms_engine_internals.md` §7.x; only the specials route is repeated here.
