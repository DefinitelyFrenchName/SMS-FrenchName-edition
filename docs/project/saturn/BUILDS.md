...
| **0.14.12** | REFsaturn+stage: `57332d87…` / hidden: `a56a9a2e…` | (this) | **HER PALETTES NOW FOLLOW THE CONFIRM BUTTON** (maintainer request: "Saturn only has her default palette out of 4"). Two things were wrong. (1) Her transform copied `$EE:C000` unconditionally, overwriting whatever slot the character select had loaded — so she looked identical on every button, and her SECOND canon Super S palette (embedded at `$EE:C020` since v0.5.0) had never once been on screen. (2) **The slot a Saturn player can reach is never 0-3.** Summoning her requires holding **L+R**, and L/R are patch 3's palette modifiers, so the reachable slots are **4-7** (measured a=4 b=5 y=6 x=7, `probe_saturn_palslot.lua`); the first cut of this fix clamped out-of-range slots to her default and reproduced the original bug exactly. The copier now reads the slot from `$1D02` (P1) / `$1D05` (P2) — the same variables the vanilla palette loader reads — and **masks** it with `SAT_PAL_SLOTS-1`, mapping every modifier combination onto her four. **Only two of her palettes are real:** Super S ships exactly two per character (both games' char-select dedup writes 0 or 1, and both loaders resolve the pointer as `selector*3` over the manifest — the other two manifest pointers are the 8-byte icon palette and the effects palette, *not* character colours). Slots 2/3 are authored by hue-rotating **only her costume ramp** (entries 1, 8-12, the exact set that differs between her two canon palettes), so skin, hair, bow and outline never move; green and gold, chosen with the maintainer against a render of her own sprite — crimson was rejected because her bow is already red (`$942020`/`$CD2020`) and the costume merged with it. Big Zam, the donor for every other character's extras, is an SMS edition with a 1-9 roster and has no Saturn to lift from. Implementation: `EE_FIGHTPALS` (4x0x20 at `$EE:C080`) + the copier switched from `lda long,X` (fixed base, cannot be indexed) to `lda [$10],Y`, with `$10-$12` saved and restored. Verified end-to-end per button: the CGRAM row is byte-identical to the intended palette for all four (A canon violet, B canon blue, Y green, X gold), and **both players were measured** — forcing `$1D02`/`$1D05` to different values proves each side reads its own variable (P2 with slot 3 renders gold while P1 stays violet). New gate check asserts the four rows are DISTINCT; negative-controlled against v0.14.9, where it fails. |

| **0.14.15** | REFsaturn+stage: `e1788e31…` / hidden: `8c5db8e4…` | (this) | **SLOT 3: GOLD -> NEAR-BLACK** (field note: gold is "fine but not ideal", low contrast against some backgrounds). Measured rather than eyeballed — her costume ramp's mean luminance against the captured stage (background 0.57): **gold 0.63, separation 0.06 — the worst of every candidate**, which is the reported wash-out. **Teal was the obvious replacement and was REJECTED on measurement:** it reads strongly against a pink stage, but that is HUE contrast, and its luminance (0.66) is barely different from gold's, so it would wash out identically on a cool-coloured stage. **Near-black separates by LUMINANCE** (0.21, separation **0.36**), which holds whatever hue the stage is, and keeps a faint violet cast so she still reads as herself. Deep grey (0.31/0.26) was the middle option and lost the violet. Judged from screenshots of a REAL match, not swatches — `probe_saturn_palshot.lua` (new) captures her in-match on a chosen palette slot, because a contrast question cannot be answered against a flat backdrop. ⚠ **One background only**: the stage-selection poke fought the match loader (`$8E` breaks the load; `$1838` alone did not take), so the comparison is a single stage — near-black is the darkest option and is the one to re-check if a very dark stage is ever added. Builder change: the authored slots are now HLS transform specs (`hue`/`lmul`/`smul`) rather than a bare hue, since this one is a luminance change. Slot 3 verified byte-identical to the intended palette in the built ROM, all four slots distinct, gate 49/49. |
| **0.16.1** | REFsaturn+stage: `c8f7dae8…` / hidden: `91639250…` | (this) | **HER TWO GROUND THROWS ARE FIXED — both faults INHERITED FROM SUPER S** (field report: "both ground throws are swapped, and the punch grab's directions are reversed too"). Reproduced before touching anything, and the mechanism measured rather than inferred — the first lead, her odd button-map record (`$C1:174E`), turned out to be **a red herring**: the +0x51 request pipeline is never written during a throw (probe_throwsrc), so its swapped pair is unrelated. What decides is a **4 x 8-byte close-throw table indexed by attack button** (`$C1:C84A`, consumed by `$C1:055A`; index 2 = HP, 3 = HK, a record's last byte = the act). **(1) Wrong buttons:** hers has the close grab (act `$68`) and the shoulder throw (act `$7B`) in each other's slots; the two 8-byte records are swapped, so each throw moves wholesale with its own range/gating fields. **(2) Wrong direction, fixed as an INPUT swap** (maintainer, after playing v0.16.0): 6 and 4 are simply read the other way round for that one throw — animation, turn-around and toss velocity all stay vanilla. The direction is the thrower's FACING (`$C1:0619 lda $50,x / and #$01 / eor #$01 / sta $09,x`), so a 19-byte stub at `$DA90` in her `$C1` copy re-inverts that bit **when DP `$07` — the act the throw record just supplied — is `$7B`**, and the call site becomes `jsr` + 5 NOPs. Scoped by ACT so it follows the throw after the swap, and confined to HER copy so vanilla throws are untouched. ⚠ **v0.16.0 got this wrong and is retired**: it negated her toss record's X velocity (`$FA80` -> `$0580`) instead. The measured outcome was identical — and it read wrong in play, because she no longer turned around. The lesson is that *matching the measurement is not the same as matching the request*: the ask was "map 6HP to 4HP", not "make the victim land in front". Both byte patterns were confirmed byte-identical in the **Super S ROM** first, so this corrects the original game, not the port. Measured matrix (thrower act + which side the victim lands): 6HP `$7B` front / 4HP `$7B` back / 6HK `$68` front / 4HK `$68` back — matching Uranus's exactly, where before HP gave `$68` and HK gave `$7B` with the sides inverted; and 6HP's trace is now the mirror of 4HP's, i.e. a pure input swap. `SATURN_THROWFIX=0` builds the old behaviour. Gate **53 checks** (four new throw assertions that check the DIRECTION as well as the act, since an act-only check passes on a build that still throws backwards); regression 57/57. |
| **0.17.0** | REFsaturn hidden: `3dee7316…` | (this) | **HER ROUND-WON BADGE — the medallion under the life bar** (maintainer report 2026-08-09: taking a round showed nothing on her side). The badge's top-left cell comes from a **ten-entry table at `$C0:E166`** indexed by `charID*2` (word = `prio<<13 | palette<<10 | tile`; the code derives the other three cells as `T+1`/`T+$10`/`T+$11`, and P2 ORs `#$0400` to move BG3 palette 6 to 7). Id `$1C` reads **0x38 past the end, into CODE at `$C0:E19E`** — measured on the unfixed build, her four cells really do hold `1E0A 1E0B 1E1A 1E1B`, pointing at tile `$20A`, past the end of the 512-tile HUD sheet; the CHR there happens to be blank, so "nothing" was luck and garbage was equally available. ⚠ **EIGHT read sites, not two** — `$DCC1`/`$DCFC` (descriptor, first draw), `$DE4D`/`$DE5D`/`$DE79`/`$DE8C` (direct `$2118` redraw), `$DFF9`/`$E034` (match end) — and **six are P2 or redraw paths**, so a two-site fix passes the one-round P1 test anyone writes first. The count was taken before a line was written (traps 5/19) and a build-time tripwire asserts **zero** surviving reads anywhere in the image. **What ships:** the table is relocated to bank `$EE` as 29 entries (id 28 = `$38CE`); the four 8-bit-X sites re-encode in place — `ldx`/`txa` zero-extends in two fewer bytes than `lda`/`and #$00FF`, which is exactly what makes the 4-byte long read fit the original 11 — and the two 16-bit-X redraw blocks become one `jsl` each to a stub reproducing them verbatim. Her four tiles go into the HUD sheet's blank window at **`$CE`/`$CF`/`$DE`/`$DF`, Super S's own slots**: the donor keeps that sheet RAW at `$E0:D2B8` while SMS compresses it at `$E0:21E6`, and **503 of its 512 tiles are byte-identical**, so this is a graft into the same sheet rather than a port — which is also why every compressed-stream search for her badge came back empty. Her icon palette (`$E0:B270`, already extracted) now rides the transform palette copier into CGRAM shadow `$0530`/`$0538`; without it the medallion wears the shell's colours and changes with the shell (the unfixed build measurably shows Uranus's). ⚠ **Trap 6 applied and answered:** the job record at `$E0:0000` (`[src16][srcbank][vram16][flags]`) carries **no length** — the DMA is sized by what the decoder wrote — so the builder asserts the re-encoded sheet still expands to exactly `0x2000`, and the gate asserts the tiles arrive identically on both sides and every shell. ⚠ **The stock encoder could not do it**: `sms_lz.encode` is literals plus one RLE trick and *expanded* the sheet `0xF31` -> `0x1B95`; new `encode_lz` (an optimal parse over the format's real back-references) gets it to `0xF0E`, under vanilla, so the edit stays in place. `encode` is untouched and patch 16 rebuilds byte-identical (`c9ad4910…` / `257598c8…`) to prove it (trap 21). `SATURN_BADGE=0` restores the old behaviour and differs from v0.16.1 by **exactly the two version-string bytes**. Gate **60 checks** (badge on both sides x three shells, each winning TWO rounds, plus cross-shell tile invariance); negative-controlled on v0.16.1, where every dimension reports BAD. |
| ~~0.15.0~~ **RETIRED** | REFsaturn+stage: `6c2b479a…` / hidden: `7c6d4968…` | (this) | **TESTED AND RETIRED THE SAME DAY.** The maintainer played it and ruled patch 17's stage "a bit distracting visually", so the tenth stage stays an OPTIONAL standalone patch and the Saturn line stays at **v0.14.15** — which the builder now reproduces **byte-for-byte** (`8c5db8e4…`/`e1788e31…`) with the hook in place but off, so the retirement is proven inert rather than asserted. The capability is kept one env var away: `SATURN_ALLSTAGES=1` folds patch 17 in and tags the on-screen version **S** (`v0.14.15HRS`) so such a build can never be mistaken for the shipped one in a field report; that build differs from the shipped v0.14.15 by exactly six bytes (patch 17's three, the version tag, the checksum). Below is what 0.15.0 was, kept because the byte accounting is the evidence for the retirement. **PATCH 17 FOLDED IN** (maintainer request: "a Saturn line 0.15 that stacks 0.14.15 and patch 17"). No Saturn code changes at all — the tenth stage (なかよし編集部) becomes selectable and joins patch 3's random-default pool. Applied by calling `mkpatch17.apply_to()` on the finished image rather than re-implementing the bytes, so there is one copy of that knowledge and one set of assertions; it runs LAST, after every Saturn edit, and its random-pool site is found BY SIGNATURE because it lives in patch 3's appended bank. **The diff against v0.14.15 is exactly ten bytes** and was checked, not assumed: `0x3BADE` (`sta $1F59` -> `stz`), `0x2800D3`/`0x2800D9` (the pool's two `#$0009` -> `#$000A`), the embedded version string `0.14.15HR` -> `0.15.0HR`, and the two checksum bytes — nothing else moved. Knobs: `SATURN_ALLSTAGES=0` builds the 0.14.15 feature set (then differing from v0.14.15 only by the version string), `SATURN_STAGE_BGM=<byte>` swaps that stage's music (its own track is `$06`; the nine normal stages hold `$0A`-`$12`). Verified on this build: ten stages reachable in the config screen (nine before), the stage loads and the menu draws its name; gate **49/49**, regression **57/57**. |
